...
- `TODO.md` 是项目功能状态的唯一来源。开始开发前检查相关任务；完成实现后同步更新状态。
- 只有同时满足以下条件才能勾选完成：功能已经接入正式代码路径；对应逻辑检查通过；已完成要求的构建、UI 或实机验收；README 与实际行为一致。
- 已实现但尚未完成实机或界面验收的项目保持未勾选，并在括号中写明“已实现，待……验收”，不得把代码完成等同于交付完成。
- 新增、取消或改变功能范围时同步维护 `TODO.md`，删除已经失效的描述；不得在 README 中把计划功能写成已发布功能。
- 每次提交前检查本次修改是否影响 `TODO.md`、README 或本文件，并在 staged diff 中确认相关状态同步；纯只读检查和无关修改不改路线图。
