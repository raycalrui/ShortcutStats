...
func runActivityExportChecks() {
    ...
    let rows = [row("active.seconds", 60), row("active.seconds", 30, hour: 1),
                ...
                row("network.download.bytes", 1024), row("network.upload.bytes", 256)]
    ...
    check(!input.contains("network.download.bytes"), "键鼠每日 CSV 不混入整机网络流量")
    let network = ActivityCSV.csv(rows: rows, kind: .network)
    check(network.contains("\"network.download.bytes\",1024.0,\"bytes\"") &&
          network.contains("\"network.upload.bytes\",256.0,\"bytes\""), "网络每日 CSV 分开导出上传下载字节")
    ...
    let hours = ActivityCSV.csv(rows: [row("key:C", 2), row("key:C", 3, hour: 1),
                                       row("network.download.bytes", 1024)], kind: .hourly)
    ...
    check(hours.contains("\"network.download.bytes\",1024.0,\"bytes\""), "小时 CSV 为网络流量保留字节单位")
