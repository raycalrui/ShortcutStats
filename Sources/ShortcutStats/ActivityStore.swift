...
struct HourMetric: Identifiable, Codable {
...
final class ActivityStore {
    ...
    /// Logical snapshot includes unflushed counters; never copy an open WAL database.
    func snapshot() throws -> [HourMetric] {
        try rows(from: "", through: "9999-12-31", appID: "")
    }

    func replace(with rows: [HourMetric]) throws {
        try execute("BEGIN IMMEDIATE")
        do {
            try execute("DELETE FROM metrics")
            var statement: OpaquePointer?
            guard sqlite3_prepare_v2(db, "INSERT INTO metrics VALUES(?,?,?,?,?,?)", -1, &statement, nil) == SQLITE_OK else { throw failure() }
            defer { sqlite3_finalize(statement) }
            for row in rows {
                sqlite3_reset(statement)
                sqlite3_bind_double(statement, 1, row.hour.timeIntervalSince1970)
